...
delElems xs ys = [y | y <- ys, not (elem y xs)]
...
connectedComponents g@(Graph (n:ns) es) = connects : connectedComponents (Graph remains es)
    where
        connects = depthFirst g n
        remains = delElems connects ns
...
main = do
         print $ connectedComponents testG
